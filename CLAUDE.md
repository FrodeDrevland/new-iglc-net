...
- Conferences in the back office (apps/conferences/admin_views.py, dashboard.py; docs/conference-sites.md): the top-level menu "Conferences" (hook register_conferences_menu_item) holds All conferences (the archive's Conference model: list, edit form, and a dashboard per conference at /manage/conferences/<pk>/ with superuser actions), Programmes, Proceedings production and Website standard pages. Archive keeps papers, authors and links. Delete a conference only through the dashboard's delete view (it cascades through Wagtail's page delete; a plain CASCADE on the foreign key would break the page tree).
